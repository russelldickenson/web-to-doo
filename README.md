...
A modern, responsive, client-side To Do application. Built purely with semantic HTML5, vanilla CSS, and vanilla JavaScript—running completely in the browser with zero backend dependencies.
...
- **Sub-tasks (Steps)**: Create, toggle completion, rename, and delete optional sub-tasks (steps) for any todo item via an always-visible checklist icon button that opens a sliding details panel.
...
- **State Persistence**: State is automatically persisted in the browser's `localStorage` (under the key `ms_todo_state`), preserving your lists and tasks between page reloads.
